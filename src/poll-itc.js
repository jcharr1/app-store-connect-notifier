...
function _checkAppStatus(currentAppInfo) {
  ...
  const buildChange = JSON.stringify(lastAppInfo ? lastAppInfo.builds : []) != JSON.stringify(currentAppInfo.builds)
  if (buildChange) {
    ...
    builds.forEach((buildInfo) => {
      ...
      if (!oldBuildInfo) {
        ...
        oldBuildInfo.external_build_state != buildInfo.external_build_state ||
        oldBuildInfo.beta_review_state != buildInfo.beta_review_state ||
        oldBuildInfo.short_version != buildInfo.short_version) {
        poster.slackBuild(currentAppInfo, buildInfo)
        newBuildInfo[buildInfo.version] = buildInfo
      } else {
