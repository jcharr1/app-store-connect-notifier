...
function slackAttachmentBuild(fallback, appInfo, buildInfo) {
  const attachment = {
    ...
    "fields": [
      ...
      {
        "title": "Version",
        "value": buildInfo.short_version || "",
        "short": true
      },
      ...((buildInfo.external_build_state || buildInfo.beta_review_state) ? [{
        "title": "TestFlight Status",
        "value": buildInfo.external_build_state || buildInfo.beta_review_state,
        "short": true
      }] : []),
      {
        "title": "Uploaded",
        "value": buildInfo.uploaded_data ? moment(buildInfo.uploaded_data).format("YYYY-MM-DD HH:mm") : "",
        "short": true
      }
    ],
    "footer": "App Store Connect",
...
module.exports = {
  ...
  // Expose for simple local testing without posting to Slack
  _slackAttachmentBuild: slackAttachmentBuild
}
